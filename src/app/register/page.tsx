...
export default function RegisterPage() {
  ...
  const { signUp, signInWithGoogleProvider, signInWithFacebookProvider } = useAuth();
    ...
    <PageShell>
      <section className="mx-auto flex max-w-7xl items-center justify-center px-4 py-16 sm:px-6 lg:px-8 lg:py-24">
        <div className="w-full max-w-md rounded-[32px] border border-slate-200 bg-white p-8 shadow-[0_30px_80px_-30px_rgba(15,23,42,0.25)]">
          <p className="text-sm font-semibold uppercase tracking-[0.25em] text-blue-600">Start hosting</p>
          <h1 className="mt-3 text-3xl font-semibold text-slate-950">Create your EventHub account</h1>
          <p className="mt-3 text-sm text-slate-600">Launch your first event and build a polished presence for your audience.</p>
          ...
          <p className="mt-6 text-sm text-slate-500">Already have an account? <Link href="/login" className="font-semibold text-blue-600">Log in</Link></p>
        </div>
      </section>
    </PageShell>
  );
